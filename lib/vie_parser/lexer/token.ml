...
let format (tok: t) : string =
  ...
  | Number (num, loc) -> Printf.sprintf "Tok { type: Number, value: %d, loc: %s }"  num (Location.format loc)
  | Decimal (num, loc) -> Printf.sprintf "Tok { type: Decimal, value: %f, loc: %s }" num (Location.format loc)
